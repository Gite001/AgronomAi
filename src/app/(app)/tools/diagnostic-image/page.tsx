...
import { analyzePlantImage, AnalyzePlantImageOutput } from "@/ai/flows/analyze-plant-image";
import Image from "next/image";
import { Alert, AlertDescription, AlertTitle } from "@/components/ui/alert";
import { Bot, Image as ImageIcon, Loader, Sparkles, X, MapPin, LocateFixed, Camera, Upload, TriangleAlert, ListPlus, ArrowLeft, Microscope } from "lucide-react";
...
export default function DiagnosticImagePage() {
  const [analysis, setAnalysis] = useState<AnalyzePlantImageOutput | null>(null);
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const input = { photoDataUri: preview };
      ...
      const entry: Omit<JournalEntry, 'id' | 'timestamp'> = { 
        tool: 'Diagnostic par Image', 
        input: journalInput, 
        output: result 
      };
      const newEntryId = addEntry(entry);
      setLastEntryId(newEntryId);
  ...
  const handleAddToPlan = () => {
    if (analysis?.suggestedAction) {
        addTask({
            ...
            sourceTool: 'Diagnostic par Image',
            sourceEntryId: lastEntryId || undefined,
        });
        toast({
            title: "Tâche ajoutée",
            description: "L'action suggérée a été ajoutée à votre plan d'action.",
        });
    }
  }
        ...
        <form onSubmit={handleSubmit}>
          <CardHeader>
            <CardTitle className="text-2xl font-headline flex items-center gap-2">
              <Microscope className="text-primary" /> Diagnostic par Image
            </CardTitle>
            <CardDescription>
              Téléchargez ou prenez une photo de votre plante et laissez notre IA l'analyser.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
             <Tabs value={inputMode} onValueChange={setInputMode} className="w-full">
                ...
                <TabsContent value="upload" className="pt-4">
                    ...
                    <Label htmlFor="plant-image">Photo de la plante</Label>
                    {preview ? (
          ...
          <CardFooter>
            <Button type="submit" disabled={!preview || loading} className="w-full">
              {loading ? (
                <>
            ...
            <div className="space-y-4">
              ...
              <Card className="bg-background/50">
                <CardHeader className="flex flex-row items-center justify-between">
                  <CardTitle className="text-base">Action Suggérée</CardTitle>
                   <Button variant="outline" size="sm" onClick={handleAddToPlan}>
